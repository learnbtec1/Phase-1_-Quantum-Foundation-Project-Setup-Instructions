...
/**
 * Browser URL for the office-room GLB (`public/models/...`).
 * Repo ships `/models/office2.glb`; legacy `/models/office.glb` may be absent locally — set
 * `NEXT_PUBLIC_OFFICE_GLB_URL=/models/office.glb` after adding that file under `public/models/`.
 */
const _officeGlbUrlTrimmed = (process.env.NEXT_PUBLIC_OFFICE_GLB_URL ?? '').trim();
export const OFFICE_GLB_PUBLIC_PATH: string =
  _officeGlbUrlTrimmed || '/models/office2.glb';

/** Shipped matte office backdrop PNG (`frontend/public/models/environments/`). */
export const AVATAR_2D_ENV_DEFAULT_IMAGE = '/models/environments/office_final.png' as const;

/**
 * Studio mode: textured `scene.background` instead of loading the office GLB.
 * `NEXT_PUBLIC_AVATAR_2D_ENV_BACKGROUND=false` restores the full 3D office room.
 */
export function readAvatarUse2dEnvBackground(): boolean {
  if (typeof process === 'undefined') return true;
  const v = (process.env.NEXT_PUBLIC_AVATAR_2D_ENV_BACKGROUND ?? '').trim().toLowerCase();
  if (v === 'false' || v === '0' || v === 'off' || v === '3d_room') return false;
  return true;
}
...
/**
 * Round carpet mesh centre in the default office shell local space (`Carpet_Carpet_0` in office_scene.default.json).
 * Avatar stands here to align with the rug in the matte backdrop.
 */
export const CARPET_MESH_LOCAL_XZ = {
  x: 4.0460638999938965,
  z: -0.46765613555908203,
} as const;

/** World XZ on the carpet centre — backdrop integration anchor. */
export function getCarpetCenterWorldXZ(): [number, number] {
  const [rx, , rz] = getRoomGroupPosition();
  const s = 1.0;
  return [rx + CARPET_MESH_LOCAL_XZ.x * s, rz + CARPET_MESH_LOCAL_XZ.z * s];
}

function defaultAvatarPositionOnCarpet(): [number, number, number] {
  const [wx, wz] = getCarpetCenterWorldXZ();
  return [wx, 1.74, wz];
}
...
/**
 * Default placement for the shipped office GLB + VRM in `AvatarCanvas` (metres, Y-up).
...
export const AVATAR_OFFICE_SCENE_DEFAULTS = {
  ...
  officeScale: 1.0,
  // Feet Y = world floor; XZ = carpet centre (matte backdrop rug).
  avatarPosition: defaultAvatarPositionOnCarpet(),
  /** Luxury office scale — uniform upscale vs rug/window/shelves (foot grounding via FloorLock). */
  avatarScale: 1.68,
  cameraPosition: [-2.85, 1.58, 1.75] as [number, number, number],
  orbitTarget: [0.15, 1.28, -0.52] as [number, number, number],
} as const;
...
/** Public URL for the 2D environment image (PNG/JPEG under `public/`). */
export function readAvatar2dEnvImageUrl(): string {
  if (typeof process === 'undefined') return AVATAR_2D_ENV_DEFAULT_IMAGE;
  const trimmed = (process.env.NEXT_PUBLIC_AVATAR_2D_ENV_URL ?? '').trim();
  return normalizePublicModelUrl(trimmed || AVATAR_2D_ENV_DEFAULT_IMAGE, AVATAR_2D_ENV_DEFAULT_IMAGE);
}
...
/**
 * VRM position when `readAvatarUse2dEnvBackground()` is on: world origin on XZ, same stance Y as office defaults
 * (`NEXT_PUBLIC_AVATAR_STAND_Y_OFFSET` still applies).
 */
export function getAvatar2dStudioScenePosition(): [number, number, number] {
  const y = AVATAR_OFFICE_SCENE_DEFAULTS.avatarPosition[1] + readAvatarStandYOffsetEnv();
  return [0, y, 0];
}
...
/**
 * When a VRMA clip drives the body, procedural layers (presence, intent motor, humanization, …)
 * used to be fully skipped. This returns **0–1** scale: how much of that organic stack is blended
 * on top of the VRMA+bind pose after intent. **0** = legacy isolate (no procedural). **1** = full stack.
 * Set `NEXT_PUBLIC_AVATAR_VRMA_PROCEDURAL_BLEED` (e.g. `0.22`–`0.38`); unset defaults to **0** (backward compatible).
 */
export function readAvatarVrmaProceduralBleed01(): number {
  if (typeof process === 'undefined') return 0;
  const raw = (process.env.NEXT_PUBLIC_AVATAR_VRMA_PROCEDURAL_BLEED ?? '').trim();
  if (!raw) return 0;
...
/** Wireframe room cage + floor footprint — verify colliders vs ROOM_BOUNDS. Default OFF. */
export function readDebugPhysicsCollidersEnv(): boolean {
  return (
    typeof process !== 'undefined' &&
    process.env.NEXT_PUBLIC_DEBUG_PHYSICS_COLLIDERS === 'true'
  );
}
 ...
 * DEFAULT and OFFICE both use the shipped office shell (`OFFICE_GLB_PUBLIC_PATH`; default `/models/office2.glb`).
 */
