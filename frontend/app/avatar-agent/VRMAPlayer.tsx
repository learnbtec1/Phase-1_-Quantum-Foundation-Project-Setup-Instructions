...
/**
 * Attempt one VRMA URL — async `loadAsync` (independent of TTS half-duplex).
 * `isTalkingRef` is driven by `speakWithTTS` / `#tts-audio`, not VRMA loader completion — avoids deadlock on slow nets.
 * `avatar:performance` / `avatar:vrma:play` cues remain best-effort; late clips apply next mixer frame.
...
export function VRMAPlayer({ vrm, vrmaActiveRef, vrmaPoseRef }: VRMAPlayerProps): null {
  ...
  useFrame((_, delta) => {
    ...
    syncMotionAuthorityFromVrma(isActive);
