...
import { isEdgePrimaryTts, setTtsPlaybackAudioElement } from '@/ai/io/tts';
...
export type AvatarSessionContext = 'default' | 'btec_evaluator';

export type AvatarAgentClientProps = {
  /**
   * صفحة /cognie: واجهة بدون أزرار (محادثة عبر الإدخال + Enter)، وتفعيل الصوت تلقائياً.
   */
  cognieCleanUi?: boolean;
  /** Landing / links: ?context=btec_evaluator — session_context for WS bridge */
  sessionContext?: AvatarSessionContext;
...
export default function AvatarAgentClient({
  ...
  useEffect(() => {
    ...
    if (isEmergencyAuthFreeze()) {
      try {
        localStorage.setItem(COGNI_ACCESS_TOKEN_KEY, getPublicDevOpaqueToken());
        notifyAuthChanged();
      } catch {
        /* ignore */
      }
  ...
  /** Hard bind DOM `#tts-audio` or fallback `Audio()` — contract: `getElementById` on mount. */
  useLayoutEffect(() => {
    if (typeof window === 'undefined') return;
    const el = document.getElementById('tts-audio');
    const audio = el instanceof HTMLAudioElement ? el : new Audio();
    if (!(el instanceof HTMLAudioElement)) {
      console.warn('[AvatarAgentClient] #tts-audio missing — programmatic Audio fallback');
    }
    audio.preload = 'auto';
  ...
  const isBtecEvaluatorEntry = sessionContext === 'btec_evaluator';
  const showMissionStrip =
    !missionDismissed &&
    ((hasTeacherDeepLinkParams && !studentOverrodeDeepLinkOnDashboard) || isBtecEvaluatorEntry);
  ...
  const evaluatorContextSentRef = useRef(false);
  useEffect(() => {
    if (!isConnected || sessionContext !== 'btec_evaluator') return;
    if (evaluatorContextSentRef.current) return;
    evaluatorContextSentRef.current = true;
    sendWsPayload({
  ...
  }, [isConnected, sessionContext, sendWsPayload]);
  ...
  // Autonomous recovery hard-fail — audible + lip path gave up; WS / TTS escalation message
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const onCritical = (): void => {
      toast.error('Audio system recovered failure', {
        description: 'Playback fell back to browser speech. If this repeats, refresh the page.',
        duration: 8000,
      });
    };
    window.addEventListener('cogni:audio:recovery:critical', onCritical);
    return (): void => {
      window.removeEventListener('cogni:audio:recovery:critical', onCritical);
    };
  }, []);
  ...
  useEffect(() => {
    if (!cognieCleanUi) return;
    void handleStartSession();
  }, [cognieCleanUi, handleStartSession]);
  ...
  return (
    ...
    <main className={shellClass} dir="rtl">
      <audio
        ...
        className="sr-only pointer-events-none fixed left-0 top-0 h-px w-px opacity-0"
        aria-hidden
      />
      ...
      {!hasStarted && cognieCleanUi && (
        <BodyPortal>
          <div
            ...
            aria-live="polite"
          >
            <p className="text-sm font-medium text-white/85">جاري تجهيز كوجني…</p>
          </div>
        </BodyPortal>
      )}
        ...
        <BodyPortal>
          ...
          <div className="bg-[#121225] border border-violet-500/20 rounded-3xl p-12 max-w-lg text-center shadow-[0_0_50px_rgba(139,92,246,0.15)]">
            ...
            <p className="text-gray-500 mb-10 text-sm leading-relaxed">
              بعد البدء تظهر نافذة المحادثة أسفل يمين الشاشة. يمكنك الكتابة دائماً؛ أما بالميكروفون فيُفعَّل{' '}
              <span className="text-gray-400">فقط</span> عندما{' '}
              <span className="text-gray-400">أنت تضغط</span> زر 🎤 — لا يُفتح تلقائياً بعد كلام كوجني.
            </p>
      ...
      <>
          ...
          {showMissionStrip && (
            ...
            >
              <span className="truncate font-medium tracking-wide">
                {isBtecEvaluatorEntry && !hasTeacherDeepLinkParams
                  ? '🎯 وضع المقيّم BTEC — كوجني كمساعد سقراطي'
          ...
          <ConversationManager
            autoUnlockOnMount={cognieCleanUi}
            onUserSpeaking={() => {
          ...
          {!cognieCleanUi ? <PermissionBanner /> : null}
            ...
            >
              ...
              >
                {history.length === 0 && (
                  <p className="px-1 text-center text-[12px] leading-relaxed text-gray-500">
                    {sessionLocked
                      ? cognieCleanUi
                        ? 'جاري تهيئة الجلسة والاتصال…'
                        : 'اضغط «ابدأ الآن» أعلاه لتفعيل المحادثة والميكروفون.'
                      : cognieCleanUi
                        ? 'اكتب في الأسفل واضغط Enter للإرسال.'
              ...
              <div className="shrink-0 border-t border-white/10 bg-black/20 px-3 py-3">
                <ChatInput
                  ...
                  hideSendButton={cognieCleanUi}
                  showWebSpeech={false}
                  placeholder={
                    cognieCleanUi ? 'اكتب رسالتك… (Enter للإرسال)' : 'Speak, or type here...'
                  }
                  className="!max-w-none w-full flex-wrap gap-2"
                  trailingSlot={
                    COGNI_MIC_UI_ENABLED && !cognieCleanUi ? (
                      <button
                        type="button"
