...
import type { CogniLipSyncProbe } from '../LipSyncManager';

/** @deprecated Prefer {@link CogniLipSyncProbe} — retained for callers that expect legacy name */
export type LipSyncProbeWin = CogniLipSyncProbe;

/** Updated every frame from LipSyncManager (viseme queue + audio playhead + morph activity). */
export function readLipSyncProbeFromWindow(): CogniLipSyncProbe | null {
  if (typeof window === 'undefined') return null;
  const p = (window as Window & { __cogniLipSyncProbe?: CogniLipSyncProbe }).__cogniLipSyncProbe;
  return p ?? null;
}
