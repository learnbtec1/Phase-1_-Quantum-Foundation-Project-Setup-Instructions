...
import {
  sampleIntentMotorMicroNoise,
  sampleIntentMotorPhraseMicroVar,
} from './motionIntentNoise';
import { MOTION_PIPELINE_DEBUG, recordIntentMotorSignature } from '@/lib/debug/motionPipelineDiag';
...
function logIntentMotorStateIfChanged(emb: EmbodimentState): void {
  ...
  recordIntentMotorSignature(sig);
  ...
  if (!active) {
    ...
    // eslint-disable-next-line no-console -- intentional intent motor diagnostics
    console.log(`[INTENT MOTOR] State: ${state} | Intent: ${intent} | Intensity: ${intensity.toFixed(3)}`);
  }
}
...
function applySemanticEmbodimentDeltas(
  ...
  if (uncertainty > 0.4) {
    mulBoneDeltaEuler(
      finalPose,
      'head',
      (Math.sin(t * 1.03 + 0.2) * 0.016 + sampleIntentMotorMicroNoise(t, 8) * 0.35) * uncertainty * m * gateUnc,
      0,
      0,
    );
  }
  ...
  /** Negation / denial — subtle horizontal head shake gated on speech phrase (kinesic “no”). */
  const neg = emb.hints.negation ?? 0;
  if (neg > 0.18 && speak) {
    const gate = phraseGate * (0.5 + 0.5 * neg);
    const shake = Math.sin(t * 5.35 + 0.33) * 0.024 * neg * m * gate;
    mulBoneDeltaEuler(finalPose, 'neck', shake * 0.38, 0, 0);
    mulBoneDeltaEuler(finalPose, 'head', shake * 0.62, 0, 0);
  }
}
...
export function applyIntentMotor(
  ...
  const microVar = sampleIntentMotorPhraseMicroVar(tScaled);
  ...
  if (intent === 'explaining') {
    ...
    const hy =
      ((Math.sin(te * 0.52) * genMul + Math.sin(beat * 0.95) * phraseMul * 0.55) * 0.007 * s + sampleIntentMotorMicroNoise(te, 0.4))
      * spikeIm
      * focusNeck
      * headEng;
    const hp =
      ((Math.sin(te * 0.41 + 0.4) * genMul + Math.cos(beat * 1.05) * phraseMul * 0.45) * 0.006 * s
        ...
        + sampleIntentMotorMicroNoise(te, 1.1))
      * spikeIm
      * focusHead
      * headEng;
    const hr = (Math.sin(te * 0.33 + 1.1) * 0.004 * s * genMul + sampleIntentMotorMicroNoise(te, 2.2)) * spikeIm * focusHead * headEng;
    ...
    const armA = (armLoopL + armBeat * 0.92 + sampleIntentMotorMicroNoise(te, 0.3) + asymOffset) * emotionMul.arms;
    ...
    const fore =
      (Math.sin(te * 0.62 + 0.3) * genMul + Math.sin(beat * 2 + 0.4) * phraseMul * 0.35 * e + sampleIntentMotorMicroNoise(te, 3.1)) *
      0.0022 *
      s *
      armExplainMul *
      modeArmMul *
  ...
  if (intent === 'thinking') {
    ...
    const slow = tt * 0.11;
    const tilt = (Math.sin(slow) + sampleIntentMotorMicroNoise(tt, 4.2) * 0.6) * 0.0055 * s * uncMul * spikeIm * focusNeck;
    const osc = (Math.sin(tt * 0.72 + 0.5) + sampleIntentMotorMicroNoise(tt, 5.1) * 0.5) * 0.0024 * s * genMul * uncMul * spikeIm * focusHead;
  ...
  if (intent === 'listening') {
    ...
    const affirmMicro =
      emb.behaviorMode === 'LISTENING' && !speak
        ...
        : 0;
    const nodBase = (Math.sin(tScaled * 0.22) + sampleIntentMotorMicroNoise(tScaled, 6.2) * 0.35) * 0.002 * s * spikeIm * focusNeck * listMul;
    ...
    const drift =
      (Math.sin(tScaled * 0.18 + 0.7) + sampleIntentMotorMicroNoise(tScaled, 7.0) * 0.35) * 0.0015 * s * genMul * spikeIm * focusHead * driftAtten * listMul
      * emotionMul.head;
...
/**
 * Non-fatal wrapper — avoids a single bad frame from bubbling out of useFrame / tearing WebGL.
 */
export function applyIntentMotorTracked(
  finalPose: BonePoseMap,
  emb: EmbodimentState,
  delta: number,
  opts: IntentMotorApplyOpts,
  ...
  try {
    applyIntentMotor(finalPose, emb, delta, opts);
  } catch (err) {
    if (typeof console !== 'undefined') {
      // eslint-disable-next-line no-console
      console.warn('[intentMotor] apply failed — frame skipped', err);
    }
  }
}
