...
const _Q_BLEED = new THREE.Quaternion();
...
/**
 * VRMA “organic bleed”: after full procedural stack, mix each bone back toward the pre-stack pose
 * so clips stay primary while micro-life remains visible. `bleed=1` leaves `mutated` unchanged (caller skips).
 */
export function applyProceduralBleedMix(
  snapshot: BonePoseMap,
  mutated: BonePoseMap,
  bleed: number,
): void {
  const t = Math.min(1, Math.max(0, bleed));
  if (t <= 1e-7 || t >= 1 - 1e-7) return;
  for (const [key, qAfter] of mutated) {
    const qBefore = snapshot.get(key);
    if (!qBefore) continue;
    _Q_BLEED.copy(qBefore).slerp(qAfter, t);
    mutated.set(key, _Q_BLEED.clone());
  }
}
