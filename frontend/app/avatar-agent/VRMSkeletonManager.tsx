...
import { applyIntentMotorTracked } from './motion/intentMotorLayer';
...
import {
  AVATAR_BEHAVIOR_SINGLE_CONTROLLER,
  readAvatarVrmaProceduralBleed01,
} from '@/config/avatar';
...
import {
  applyRealismMicroExpressions,
  getRealismNeckBiasesRadians,
  tickRealismEngine,
} from '@/lib/avatar/realismEngine';
...
/** `NEXT_PUBLIC_MOTION_PATH_LEAK_DEBUG`: throttled blend + neck euler probe */
let __motionPathLeakDbgNextMs = 0;
const MOTION_PATH_LEAK_DEBUG =
  typeof process !== 'undefined' &&
  process.env.NEXT_PUBLIC_MOTION_PATH_LEAK_DEBUG === 'true';

/** One-shot dev explanation when procedural bleed merges with VRMA. */
let __vrmaOrganicBleedDevNoted = false;
...
export function VRMSkeletonManager({
  ...
  useFrame((_, delta) => {
    ...
    tickRealismEngine({
      ...
      vrm,
      speaking,
      emotion: String(brainSnap.emotionLabel ?? 'neutral'),
      deltaSec: safeDelta,
    });
    ...
    if (motionSource === 'IDLE' && (prevMotionSourceRef.current === 'GESTURE' || prevMotionSourceRef.current === 'VRMA')) {
    ...
    /** بعد انتهاء GESTURE / VRMA → IDLE: يبطّئ slerp الأذراع قليلاً ثم يصل إلى 1 (ease-out). */
    ...
    const rNeckBias = getRealismNeckBiasesRadians();
    const gYaw =
    ...
    const vrmaOrganicBleed =
      motionSource === 'VRMA' ? readAvatarVrmaProceduralBleed01() : 1;
    const vrmaProceduralIsolated =
      motionSource === 'VRMA' && vrmaOrganicBleed <= 1e-6;
    const needsVrmaOrganicMix =
      motionSource === 'VRMA' &&
      vrmaOrganicBleed > 1e-6 &&
      vrmaOrganicBleed < 1 - 1e-6;
    const vrmaOrganicSnapshot = needsVrmaOrganicMix
      ? clonePoseMap(finalPose)
      : undefined;

    if (
      !__vrmaOrganicBleedDevNoted &&
      motionSource === 'VRMA' &&
      vrmaOrganicBleed > 1e-6 &&
      process.env.NODE_ENV === 'development' &&
      DEBUG_MOTION
    ) {
      __vrmaOrganicBleedDevNoted = true;
      // eslint-disable-next-line no-console -- deliberate integration breadcrumb (DEBUG_MOTION only)
      console.info(
        '[VRMSkeletonManager] VRMA procedural bleed enabled — organic stack mixes at ratio',
        vrmaOrganicBleed,
        '(set NEXT_PUBLIC_AVATAR_VRMA_PROCEDURAL_BLEED; 0 restores legacy isolation)',
      );
    }
    ...
    runWithProceduralSuppression(proceduralSuppressionKeys, () => {
      if (!vrmaProceduralIsolated) {
        ...
        applyIntentMotorTracked(finalPose, embFrame, safeDelta, {
          motionSource,
          gestureLayerW,
          onMotionApplied: () => {
            recordActivity();
      ...
      if (vrmaOrganicSnapshot !== undefined && motionSource === 'VRMA') {
        applyProceduralBleedMix(vrmaOrganicSnapshot, finalPose, vrmaOrganicBleed);
      }
    });
    ...
    applyRealismMicroExpressions(vrm, {
      speaking,
      emotion: String(useBrainStore.getState().emotionLabel ?? 'neutral'),
      deltaSec: safeDelta,
    });
    ...
    if (MOTION_PATH_LEAK_DEBUG && typeof performance !== 'undefined') {
      ...
      if (tn >= __motionPathLeakDbgNextMs) {
        __motionPathLeakDbgNextMs = tn + 500;
        const neck = neckRef.current;
        // eslint-disable-next-line no-console -- motion-path diagnostics
        console.log('[motion-path-leak]', {
          weights: {
            idle: idleLayerW,
            generative: generativeLayerW,
            gesture: gestureLayerW,
          ...
          motionSource,
          vrmaProceduralIsolated,
          vrmaOrganicBleed:
            motionSource === 'VRMA' ? +vrmaOrganicBleed.toFixed(3) : 'n/a',
          speechActiveRmsGate: speechDriveSnap.active,
          speaking,
          neckRotY: neck ? neck.rotation.y : null,
        });
      }
    }
