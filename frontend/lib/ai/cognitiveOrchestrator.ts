...
/** Inline `[POSE:…]` from prepared TTS line — cleared when speech hints reset. */
let utterancePoseIntentOverride: CognitiveIntentHint | null = null;

export function setUtterancePoseIntentOverride(intent: CognitiveIntentHint | null): void {
  utterancePoseIntentOverride = intent;
}
...
export function deriveEmbodimentFromLLM(input: CognitiveOrchestratorInput): Partial<EmbodimentState> {
  ...
  } else if (utterancePoseIntentOverride && input.isAgentSpeaking) {
    /** Explicit script / LLM line tags beat heuristics, under structured LLM intent above. */
    activeIntent = utterancePoseIntentOverride;
...
export function resetCognitiveOrchestratorSmoothing(): void {
  ...
  lastStructuredCognitiveAtMs = 0;
  utterancePoseIntentOverride = null;
}
