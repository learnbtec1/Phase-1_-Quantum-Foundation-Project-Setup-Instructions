...
export function deriveSpeechSemanticHints(text: string | undefined): SpeechSemanticHints {
  if (text === undefined || !String(text).trim()) {
    return { emphasis: 0, question: 0, uncertainty: 0, explanation: 0, negation: 0 };
  }
  ...
  let negation = 0;
  if (
    /\b(no|not|never|don't|doesn't|didn't|isn't|aren't|wasn't|weren't|can't|cannot|won't|nor|neither|wrong|incorrect|false|deny|reject)\b/i.test(
      lower,
    )
  ) {
    negation += 0.42;
  }
  if (/(^|\s)(لا[\s،]|ليس\b|لست\b|لن\b|ما\s|غلط|غير صحيح|خطأ|مش\s|مو\s|كلا\b|باطل|رفض)/u.test(raw)) {
    negation += 0.44;
  }
...
export function stepSmoothedSemanticHints(target: SpeechSemanticHints, delta: number): SpeechSemanticHints {
  ...
  const tNeg = target.negation ?? 0;
  const sNeg = smoothedSemanticHints.negation ?? 0;
