...
/** Only stretch cues when cumulative span error exceeds 25% (avoid unnatural linear remap). */
const STRETCH_MIN_REL_ERROR = 0.25;
...
export function stretchVisemeCuesToDuration(
  ...
  if (Math.abs(1 - scaleRaw) <= STRETCH_MIN_REL_ERROR) {
    return cues;
  }
