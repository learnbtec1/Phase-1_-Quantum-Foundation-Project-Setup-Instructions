...
import { cogniVerbose } from '@/lib/cogniVerbose';
...
export class AgentDirector {
  ...
  start(): void {
    ...
    this._running = true;
    cogniVerbose('%c[AgentDirector] STARTED — Phase 4 orchestration active', 'color:#7c3aed;font-weight:bold');
    devLog(
      'persona active',
      COGNI_PERSONA.id,
      'deliberation×',
  ...
  async scheduleTTS(
    ...
    console.log('TTS_INPUT_TEXT:', text);
      ...
      this._later(delay, async () => {
        ...
        try {
          const audio = await speakWithTTS(text, {
            ...
            // speakWithTTS dispatches avatar:speak:start before onStart — omit duplicate emit (was doubling half-duplex VAD pause logs).
            onStart: () => {},
            onEnd: () => {
          ...
          if (!audio) {
            ...
            if (typeof window !== 'undefined') {
              window.dispatchEvent(new CustomEvent('cogni:half_duplex:reply_chain_complete'));
            }
