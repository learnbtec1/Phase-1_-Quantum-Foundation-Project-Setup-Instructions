...
export class EmotionalMemoryManager {
  ...
  recordMoment(userMood: EmotionLabel, topic?: string): void {
    ...
    if (
      ...
      cogniVerbose(`[EmotionalMemory] → Promoted to LTM: "${description}"`);
    }
  ...
  getTrajectory(): EmotionalTrajectory {
    ...
    if (entries.length < 2) {
      cogniVerbose('[EmotionalMemory] getTrajectory — insufficient data, returning stable_neutral');
      return {
    ...
    cogniVerbose('[EmotionalMemory] Trajectory:', JSON.stringify(trajectory));
    return trajectory;
  }
  ...
  getContextSummary(): string {
    ...
    cogniVerbose('[EmotionalMemory] Context summary:', summary);
    return summary;
  }
