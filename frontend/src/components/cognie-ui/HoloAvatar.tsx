...
/** Cognie Vision — أفاتار ARIA الهولوغرافي (عرض فقط). */
export function HoloAvatar({
  ...
  const imgSrc =
    typeof ariaAvatar === 'object' &&
    ariaAvatar !== null &&
    'src' in (ariaAvatar as object)
      ? String((ariaAvatar as { src: string }).src)
      : String(ariaAvatar);
  ...
  return (
    ...
    >
        ...
        <>
          ...
          <div
            ...
            style={{
              inset: -8,
              background:
                'radial-gradient(circle, color-mix(in oklab, var(--neon-cyan) 30%, transparent) 0%, transparent 70%)',
              filter: 'blur(20px)',
            }}
          />
        </>
      ...
      <div
        className="animate-float-y relative z-10 overflow-hidden rounded-full"
        style={{
          width: size - 32,
          height: size - 32,
          boxShadow:
      ...
      {caption && (
        <p
          className="absolute -bottom-6 text-[10px] uppercase tracking-[0.3em]"
