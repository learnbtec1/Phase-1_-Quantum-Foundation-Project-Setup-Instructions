...
export default function CriteriaAnalysisBox({ evaluationResult }: Props) {
  ...
  const getGradeColor = (grade?: string) => {
    ...
    if (grade.includes('FAIL')) return 'text-red-500';
    ...
    <div className="mt-8 space-y-6" dir="rtl">
      ...
      <div className="space-y-4">
        {criteriaEntries.map(([code, result]) => {
          const { achieved, feedback, evidence_quote, missing_requirements } = result;
          ...
          return (
            <div key={code} className={`p-6 rounded-xl border ${cardBorder} transition-colors duration-300`}>
              ...
              <div className="text-gray-300 leading-relaxed space-y-3">
                ...
                {evidence_quote && (
                  <blockquote className="border-r-4 border-gray-600 pr-4 py-2 text-gray-400 italic text-sm bg-black/20 rounded">
                    "{evidence_quote}"
                  </blockquote>
                )}
