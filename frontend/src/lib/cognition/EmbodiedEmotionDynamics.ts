...
export function sampleEmotionBodyDynamics(): EmotionBodyDynamics {
  ...
  const valence = THREE_CLAMP((pad?.valence ?? 0) + 1, 0, 2) / 2;

  return {
    hesitation: THREE_CLAMP(1 - arousal * 0.85 + (s.thinking ? 0.22 : 0), 0, 1),
    warmth: THREE_CLAMP(0.45 + valence * 0.5 + (s.physical?.isListening ? 0.18 : 0), 0, 1),
    attentiveness: THREE_CLAMP(0.42 + (s.physical?.isListening ? 0.35 : 0) + arousal * 0.12, 0, 1),
    confidence: THREE_CLAMP(0.48 + arousal * 0.28 + valence * 0.15, 0, 1),
    curiosity: THREE_CLAMP(0.36 + arousal * 0.22 + valence * 0.18, 0, 1),
    calmAuthority: THREE_CLAMP(0.62 - arousal * 0.25 + valence * 0.1, 0, 1),
    engagement: THREE_CLAMP(arousal * 0.55 + valence * 0.35 + (s.physical?.isListening ? 0.15 : 0), 0, 1),
