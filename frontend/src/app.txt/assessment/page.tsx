...
export default function AssessmentPage() {
  ...
  // Derived Data
  const availableSections = useMemo(() => selectedClass ? Object.keys(ACADEMIC_DATA[selectedClass] || {}) : [], [selectedClass]);
  const availableSubjects = useMemo(() => (selectedClass && selectedSection) ? ACADEMIC_DATA[selectedClass][selectedSection] || [] : [], [selectedClass, selectedSection]);
  ...
  const handleCheckPlagiarism = async () => {
    ...
    } catch (e) { alert("Error checking plagiarism"); setShowPlagModal(false); } finally { setPlagiarismLoading(false); }
  };
                ...
                <div className="relative bg-gray-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-wrap justify-between items-center gap-8">
                        <div>
                            ...
                            <div className={`text-6xl font-extrabold ${result.data.final_grade === 'REFER (FAIL)' ? 'text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'text-green-400 drop-shadow-[0_0_10px_rgba(74,222,128,0.5)]'}`}>
        ...
        {showPlagModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div className="relative bg-gray-900 border border-gray-600 w-full max-w-3xl rounded-3xl p-8 shadow-2xl transform transition-all scale-100">
                    <button onClick={() => setShowPlagModal(false)} className="absolute top-4 left-4 text-gray-400 hover:text-white">✕</button>
