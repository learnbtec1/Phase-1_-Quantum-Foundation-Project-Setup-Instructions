...
import { 
  Rocket, Wheat, Bot, 
  ArrowRight, Sparkles, Fingerprint, Activity 
} from "lucide-react";

// استدعاء المكونات التفاعلية
import NexusHero3D from "../components/NexusHero3D"; // Adjusted relative path
import DrAhmedOrb from "@/components/DrAhmedOrb";

// --- مكون البطاقة التفاعلية بميزة الـ 3D Tilt ---
function InteractiveTile({ href, title, subtitle, icon: Icon, color, delay, isLarge = false }: any) {
  ...
  return (
    <Link href={href} className={isLarge ? "md:col-span-2 row-span-2" : ""}>
      ...
      >
        {/* تأثير الإضاءة المتبع للماوس داخل البطاقة */}
        ...
        <div className="relative h-full p-8 flex flex-col justify-between z-10 relative-transform" style={{ transform: "translateZ(50px)" }}>
          <div>
            <div className={`w-14 h-14 rounded-2xl mb-6 flex items-center justify-center bg-gradient-to-br border border-white/10 group-hover:scale-110 transition-transform duration-500 shadow-lg`}
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#020617] text-white selection:bg-cyan-500/30 overflow-hidden relative font-cairo">
      
      {/* 1. المحرك البصري 3D */}
      <NexusHero3D />

      {/* 2. طبقة الضباب السينمائي */}
      <div className="fixed inset-0 bg-gradient-to-b from-transparent via-transparent to-[#020617] z-0 pointer-events-none" />

      {/* 3. واجهة المستخدم (The Dashboard Layer) */}
      <main className="relative z-10 container mx-auto px-6 py-20 min-h-screen flex flex-col">
        ...
        <div className="flex flex-col items-center text-center mb-24 pt-10">
          ...
          >
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-b from-white/40 to-white/10">LEARNING</span>
          </motion.h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto w-full mb-32">
          ...
          <InteractiveTile 
            ...
            subtitle="بيئة ثلاثية الأبعاد بالكامل للتحكم في الشركات واتخاذ قرارات مصيرية في عالم افتراضي."
            icon={Rocket}
            color="#06b6d4"
            delay={0.1}
          />
          ...
          <InteractiveTile 
            href="/ai-teacher"
            title="المعلم الذكي"
            subtitle="مساعدك الشخصي لتحليل نقاط الضعف وتطوير المهارات."
            icon={Bot}
            color="#f59e0b"
            delay={0.3}
          />
          ...
          <InteractiveTile 
            ...
            subtitle="نماذج الأعمال المتقدمة في قطاع الزراعة."
            icon={Wheat}
            color="#10b981"
            delay={0.4}
          />
        ...
        <footer className="mt-auto pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-bold text-gray-500 tracking-[0.2em] uppercase">
          ...
          <div className="flex gap-8">
            <span className="hover:text-white cursor-pointer transition-colors">Core Status: Optimal</span>
            <span className="hover:text-white cursor-pointer transition-colors">Neural Uplink: Active</span>
          </div>
        </footer>
      </main>

      {/* 4. أورب د. أحمد (الذكاء الاصطناعي العائم) */}
      <DrAhmedOrb />
    </div>
  );
}