...
export default function AiTeacherChat() {
  ...
  */
  ...
  const handleSend = async () => {
    ...
    // ── Try /api/chat first; fall back to local responses ──────────────────
    const fetchReply = async (): Promise<string> => {
      try {
        const res = await fetch('/api/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message: input, mode: 'teacher', context: analysis.context }),
        ...
        // Network error, API unavailable, or timeout — fall through to local
      }
      return getPersonalizedResponse(input, analysis.level, analysis.context);
    };
