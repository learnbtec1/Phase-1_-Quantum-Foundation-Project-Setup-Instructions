...
const DEFAULTS: Required<RigOptions> = {
  ...
  blinkPerMinMin: 12, // Corresponds to 5 seconds per blink
  blinkPerMinMax: 30, // Corresponds to 2 seconds per blink
...
export class HumanizationRig {
  ...
  private updateBreath(dt: number) {
    ...
    // Combine multiple sine waves for more natural breath, modulated by simplex noise
    const breath1 = Math.sin(this.t * Math.PI * 2 * this.breathFreq) * 0.018;
    const breath2 = Math.sin(this.t * Math.PI * 2 * (this.breathFreq * 1.5)) * 0.004;
    const breath3 = Math.sin(this.t * Math.PI * 2 * (this.breathFreq * 0.5)) * 0.002;
    const noiseMod = this.noise2D(this.t * 0.05, 0) * 0.003;
    const breath = breath1 + breath2 + breath3 + noiseMod;
  ...
  private updateGazeAndHead(dt: number) {
    ...
    // Add micro-saccades using simplex noise for more natural, continuous, and random eye movement
    const saccadeSpeed = 0.3;
    const saccadeAmount = 0.04;
    const jx = this.noise2D(this.t * saccadeSpeed, 10) * saccadeAmount;
    const jy = this.noise2D(this.t * saccadeSpeed, 20) * saccadeAmount;
  ...
  private updateIdle(dt: number) {
    void dt;
    // Use Simplex Noise for more natural and random idle sway
    const sway = this.noise2D(this.t * 0.2, this.idlePhase * 0.1) * 0.008;
