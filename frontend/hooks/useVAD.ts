...
import { ensureAudioUnlocked, getSharedAudioContext } from '@/lib/audio/avatarAudioContext';
import { buildWavBlobFromMediaChunks } from '@/lib/audio/buildWavFromMediaChunks';
import { cogniDuplexIsSpeaking, cogniDuplexIsWithinPostTtsVadCooldown, cogniDuplexTryInit, COGNI_duplex_POST_TTS_VAD_COOLDOWN_MS } from '@/lib/audio/cogniDuplexGate';
import {
...
/** One-shot UX when mic capture is impossible — distinct from avatar/TTS playback. */
let _vadMicAlertShown = false;

function showVadUnavailableAlert(kind: 'no_device' | 'permission' | 'security'): void {
  if (typeof window === 'undefined' || _vadMicAlertShown) return;
  _vadMicAlertShown = true;
  const lines =
    kind === 'security'
      ? 'Serve this app over HTTPS or localhost — insecure origins often block microphone access.'
      : kind === 'permission'
        ? 'The browser denied microphone permission. Use the lock icon in the address bar → Site settings → Microphone → Allow.'
        : 'No microphone was found or it is unplugged/disabled in Windows sound settings.';
  window.dispatchEvent(
    new CustomEvent('cogni:vad:unavailable', { detail: { kind } }),
  );
  alert(
    `Voice capture unavailable (${kind}).\n\n${lines}\n\nYou can keep chatting by typing — avatar speech does not depend on the microphone.`,
  );
}
...
export interface UseVADOptions {
  ...
  lang?: string;
  /**
   * When true (Cognie voice turn-taking): after a valid speech segment is encoded and `onSpeechEnd` resolves,
   * fully stop the microphone stream (same teardown as manual stop — no recorder restart until user toggles mic on again).
   * Does not change WS/TTS; only capture lifecycle.
   */
  stopMicAfterUtterance?: boolean;
  /** After internal teardown when `stopMicAfterUtterance` is true — sync host UI/parent refs (optional). */
  onSessionClosedFromAutoStop?: () => void;
}
...
/** Mono PCM16 WAV @ sampleRate Hz (default pipeline: 16 kHz for Whisper). */
async function buildWavBlob(chunks: Blob[], sampleRate: number): Promise<Blob | null> {
  return buildWavBlobFromMediaChunks(chunks, sampleRate);
}
...
export function useVAD({
  ...
  lang = 'ar-SA',
  stopMicAfterUtterance = false,
  onSessionClosedFromAutoStop,
  ...
  const stopMicAfterUtteranceRef = useRef(false);
  const onSessionClosedFromAutoStopRef = useRef<(() => void) | undefined>(undefined);
  stopMicAfterUtteranceRef.current = stopMicAfterUtterance;
  onSessionClosedFromAutoStopRef.current = onSessionClosedFromAutoStop;
  const isSupported =
  ...
  /** Latest stopListening — used from async MediaRecorder.onstop without stale closure. */
  ...
  const startListening = useCallback(async () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (cogniDuplexIsSpeaking()) {
        console.warn('[useVAD] startListening skipped — duplex TTS speaking lock active');
        console.log('[VAD BLOCKED]', true);
        return;
      }
      const audioGate = getTtsPlaybackElementForVadGate();
      let stabilityAttempt = 0;
      // Cooldown vs HTMLMediaElement settling — retries only when unstable, not during cooldown-only.
      while (true) {
        const st = window.__cogniState;
        const endMs = st?.lastOutboundTtsEndWallMs ?? 0;
        const inCooldown = typeof endMs === 'number' && endMs > 0 && cogniDuplexIsWithinPostTtsVadCooldown();
    ...
    try {
      ...
      const wireRecorder = (rec: MediaRecorder) => {
        ...
        rec.onstop = async () => {
          ...
          let outbound: Blob | null =
            wav && wav.size > 0
              ? wav
              : new Blob(savedChunks, { type: savedChunks[0]?.type ?? 'audio/webm' });
          if (wav && wav.size > 0) {
            console.log(
              `[VAD] WAV ready — invoking onSpeechEnd | size=${outbound!.size} bytes type=${wav.type || 'audio/wav'}`,
            );
            console.log('📦 [VAD] Speech ended - WAV blob generated, size:', outbound!.size);
          } else {
            console.warn(
              `[VAD] buildWavBlob empty/failed — invoking onSpeechEnd with raw WebM blob | size=${outbound!.size} type=${outbound!.type}`,
            );
            console.log('📦 [VAD] Speech ended — raw blob (fallback), size:', outbound!.size);
          }
          try {
            await Promise.resolve(onSpeechEnd?.(outbound!));
          } catch (onEndErr: unknown) {
            console.error('[useVAD] onSpeechEnd rejected:', onEndErr);
          }

          queueMicrotask(() => {
            if (stopMicAfterUtteranceRef.current) {
              console.log('[VAD] Auto turn-taking: tearing down microphone after utterance sent');
              stopListeningRef.current();
              queueMicrotask(() => {
                try {
                  onSessionClosedFromAutoStopRef.current?.();
                } catch (cbErr: unknown) {
                  console.warn('[useVAD] onSessionClosedFromAutoStop:', cbErr);
                }
              });
              return;
            }
      ...
      if (e?.name === 'NotFoundError' || e?.message?.includes('device not found')) {
        ...
        showVadUnavailableAlert('no_device');
        return;
        ...
        showVadUnavailableAlert('permission');
        return;
        ...
        setPermissionDenied(true);
        permissionDeniedRef.current = true;
        showVadUnavailableAlert('security');
        return;
      }
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (wav) {
          ...
          console.warn('[useVAD] Manual: buildWavBlob failed — sending raw WebM blob');
          console.log('📦 [VAD] Manual speech ended — raw blob (fallback), size:', raw.size);
          onSpeechEnd?.(raw);
        }
      };
