...
async def _get_cogni_response(message: str, context: dict) -> str:
    ...
    if _fs and str(_fs).strip():
        system_content += (
            "\n\n## تركيز المادة (مرن)\n"
            f"الجلسة مرتبطة بـ: «{str(_fs).strip()[:200]}».\n"
            "• حاول دائماً ربط الأمثلة والشرح بهذا العنوان.\n"
            "• إن سأل الطالب عن موضوع مجاور (مثل التسويق، خدمة العملاء، إدارة الأعمال) فأجب بشكل طبيعي واربطه بالموضوع الرئيسي من باب التوسيع والتعمق.\n"
            "• إن كان السؤال بعيداً تماماً عن إدارة الأعمال، أجب إجابة مختصرة ثم اقترح العودة بسلاسة: "
            "«كمان نقدر نربط هاد الشي بـ[الموضوع]. شو رأيك نكمل من هون؟»\n"
            "• لا ترفض الإجابة كلياً — دورك مدرّس يوسّع الأفق لا فلتر يغلق الأبواب.\n"
        )
