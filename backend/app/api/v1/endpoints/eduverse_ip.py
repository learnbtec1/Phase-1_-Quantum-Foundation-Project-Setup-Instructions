# -*- coding: utf-8 -*-
"""
EDUVERSE IP Endpoints v1.0
-----------------------
المسارات التي تُحوّل نظام EDUVERSE من "أداة تقييم" إلى "وكيل أكاديمي قانوني قابل للترخيص".

المسارات:
  POST   /api/v1/eduverse/grade-verified          — تقييم كامل + ناقد داخلي + ذاكرة + مشاعر + Audit
  POST   /api/v1/eduverse/appeal                  — اعتراض الطالب + إعادة الفحص
  GET    /api/v1/eduverse/student/{id}/history    — سجل الطالب الطولي
  GET    /api/v1/eduverse/audit/{job_id}          — سجل XAI كامل
"""
...
from app.api.v1.dependencies.phase2_gates import gate_llm_http_user, gate_eduverse_read_user
...
from app.archive.avatar_emotion_engine import generate_emotion_metadata
...
logger = logging.getLogger("eduverse.ip")

router = APIRouter(prefix="/api/v1/eduverse", tags=["EDUVERSE IP"])
...
async def grade_verified(
    req: VerifiedGradeRequest,
...
async def get_student_history(
    student_id: str,
    _user: User = Depends(gate_eduverse_read_user),
):
