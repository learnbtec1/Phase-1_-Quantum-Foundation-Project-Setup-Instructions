...
@pytest.mark.asyncio
async def test_short_text_returns_low_score(guard):
    result = await guard.evaluate("نص قصير جداً")
...
@pytest.mark.asyncio
async def test_long_clean_text_returns_some_score(guard):
...
@pytest.mark.asyncio
async def test_ai_markers_increase_score(guard):
