# 🌌 EDUVERSE ACADEMY - Grand Context Prompt
...
```
═══════════════════════════════════════════════════════════════
🎓 EDUVERSE ACADEMY v3.0 - Development Context
═══════════════════════════════════════════════════════════════

I am developing a high-end BTEC educational platform called "Eduverse Academy".
...
3. localStorage keys: IMMUTABLE (eduverse-auth, eduverse-assessments, eduverse-vr, btec_platform_progress)
...
│   ├── useAuth.ts                 # Auth store (eduverse-auth)
│   ├── useAssessment.ts           # Grading store (eduverse-assessments)
│   ├── useVR.ts                   # VR progress (eduverse-vr)
│   └── ...
│
...
export const useNewFeatureStore = create<NewFeatureState>()(
  persist(
    ...
    { 
      name: 'eduverse-new-feature'  // ⚠️ Choose a unique key
    }
  )
);
...
export default function NewComponent({ 
  ...
  return (
    <motion.div
      ...
      className="glass-eduverse p-6 rounded-2xl border border-white/10"
    >
...
```css
/* Glassmorphism */
.glass-eduverse     /* Frosted glass effect with neon border */
.card-eduverse      /* Glass card with hover scale */
...
const STORAGE_KEYS = {
  AUTH: 'eduverse-auth',              // User session + role
  ASSESSMENTS: 'eduverse-assessments', // Grading history
  VR: 'eduverse-vr',                  // VR progress (4 evidence items)
...
**💡 Pro Tip**: Bookmark this file and copy the template before every Copilot session for consistent, accurate code generation aligned with Eduverse Academy's architecture.
