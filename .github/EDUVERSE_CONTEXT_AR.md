...
3. مفاتيح localStorage: ثابتة (eduverse-auth, eduverse-assessments, eduverse-vr, btec_platform_progress)
...
export const useNewFeatureStore = create<NewFeatureState>()(
  persist(
    ...
    { name: 'eduverse-new-feature' }  // ⚠️ اختر مفتاحاً فريداً
  )
);
...
```css
.glass-eduverse     /* تأثير زجاج مصنفر مع حدود نيون */
.card-eduverse      /* بطاقة زجاجية مع تحجيم عند التمرير */
...
const STORAGE_KEYS = {
  AUTH: 'eduverse-auth',              // جلسة المستخدم + الدور
  ASSESSMENTS: 'eduverse-assessments', // سجل التقييمات
  VR: 'eduverse-vr',                  // تقدم VR (4 أدلة)
...
| الموضوع | الملف | الوصف |
|---------|-------|--------|
| 🇬🇧 نسخة إنجليزية | `EDUVERSE_CONTEXT.md` | السياق الكامل بالإنجليزية |
| 🇸🇦 نسخة عربية | `EDUVERSE_CONTEXT_AR.md` | هذا الملف |
